package es.klondike.utils;

public interface Constantes {
	
	//Game
	public static final int NUMBER_MINIMUM_CARDS_MOVE_DECK_TO_WASTE = 1;
    //FOURCOLOURS OR TABELAUS
	public static final int NUMBER_TABLEAUS = 7;
	
	public static final int NUMBER_FOUNDATIONS = 4;
	public static final int NUMBER_CARDS_INITIAL_FOUNDATIONS = 0;
	//DECKS
	public static final int NUMBER_CARDS_DECK_INITIAL = 24;
	public static final int NUMBER_TOTAL_INITIAL_CARDS_TABLEAU = 28;
	public static final int NUMBER_TOTAL_CARDS= 52;
	
	//waste
	public static final int NUMBER_CARDS_INITIAL_WASTE = 0;
	
	//CARDS
	public static final boolean COVERED_CARD = true;
	public static final boolean UNCOVERED_CARD = false;
	
	// 

	public static final int NUMBER_EMPTY = 0;
	public static final int ORDINAL_MINIMUN_CARD = 1;
	public static final int ORDINAL_MAXIMUN_CARD = 13;
	
	
	
	
}
